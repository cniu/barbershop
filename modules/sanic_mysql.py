...
class SanicMysql:
    ...
    async def start(self, _app, loop):
        ...
        async def _query_select(sqlstr, args=None):
        ...
        async def _query_other(sqlstr, args=None):
            async with _mysql.acquire() as conn:
                async with conn.cursor() as cur:
                    final_str = cur.mogrify(sqlstr, args)
                    # log.info('mysql query [{}]'.format(final_str))
                    await cur.execute(final_str)
                    value = await conn.commit()
                    return value

        setattr(_mysql, 'query_select', _query_select)
        setattr(_mysql, 'query_other', _query_other)
